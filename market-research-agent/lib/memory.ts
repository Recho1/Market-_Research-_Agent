...
function saveLongTermStore() {
  try {
    ...
    console.warn("[Memory] Failed to persist long-term memory:", err);
  }
}
...
// Auto-save long-term memory every 5 minutes
setInterval(() => {
  saveLongTermStore();
}, 5 * 60 * 1000);
...
export function getUserMemorySummary(userId: string): string {
  const mem = getLongTermMemory(userId);
  if (!mem.length) return "No previous research history.";
  return mem
    .slice(-10)
    .map(m => `[${m.importance}] ${m.topic}: ${m.summary}`)
    .join("\n");
}
...
export function buildMemoryContext(sessionId: string, userId?: string): string {
  ...
  if (s.shortTermMemory.length > 0) {
    const recent = s.shortTermMemory
      .slice(-5)
...
export function saveFeedback(entry: FeedbackEntry) {
  ...
  // Persist positive feedback topics to long-term memory
  if (entry.rating === "up" && entry.sessionId) {
    addLongTermMemory(
      entry.sessionId,
      "Successful query",
      `User found this helpful: "${entry.query.slice(0, 80)}"`,
      "medium"
    );
  }
}
