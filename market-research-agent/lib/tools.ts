...
export const competitorAnalysisTool = new DynamicStructuredTool({
  ...
  async func({ industry, region, topN }) {
    const sources = await serperSearch(
      `top ${topN} competitors ${industry} ${region} 2025`, 8
    );
...
export const marketSizingTool = new DynamicStructuredTool({
  ...
  async func({ market, metric, timeframe }) {
    const sources = await serperSearch(
      `${market} market size ${metric} ${timeframe}`, 6
    );
...
// ── Tool 7: News Sentiment Analysis ──────────────────────────────────────────
export const newsSentimentTool = new DynamicStructuredTool({
  name: "news_sentiment",
  description: "Analyze news sentiment and media coverage for a company or industry. Returns sentiment score, key themes, and recent headlines.",
  schema: z.object({
    subject: z.string().describe("Company name, industry, or topic to analyze"),
    timeframe: z.string().default("last 7 days").describe("Time period for news analysis"),
  }),
...
export const industryReportsTool = new DynamicStructuredTool({
  ...
  description: "Fetch and summarize professional industry reports, whitepapers, and analyst research from top sources like McKinsey, Gartner, Deloitte, and PwC.",
  schema: z.object({
...
// ── Tool 9: Currency & Exchange Rates ─────────────────────────────────────────
export const currencyExchangeTool = new DynamicStructuredTool({
  name: "currency_exchange",
  description: "Get live currency exchange rates and analyze forex implications for international market entry or investment decisions.",
  schema: z.object({
...
export const TOOL_CONFIGS = [
  ...
  {
    name: "competitor_analysis" as const,
    label: "Competitor Analysis",
    icon: "[C]",
    description: "Map competitive landscapes",
    category: "core",
    enabled: true,
  },
  {
    name: "market_sizing"       as const,
    label: "Market Sizing",
    icon: "[M]",
    description: "TAM/SAM/SOM projections",
    category: "core",
    enabled: true,
  },
  {
    name: "trend_analysis"      as const,
    label: "Trend Analysis",
    icon: "[T]",
    description: "Emerging shifts and dynamics",
    category: "core",
    enabled: true,
  },
  {
    name: "financial_data"      as const,
    label: "Financial Data",
    icon: "[F]",
    description: "Live stock prices and metrics",
    category: "core",
    enabled: true,
  },
  {
    name: "swot_analysis"       as const,
    label: "SWOT Analysis",
    icon: "[W]",
  ...
  {
    name: "news_sentiment"      as const,
    label: "News Sentiment",
    icon: "[N]",
    description: "Media sentiment and coverage",
    category: "plugin",
    enabled: false,
  },
  {
    name: "industry_reports"    as const,
    label: "Industry Reports",
    icon: "[R]",
    description: "McKinsey, Gartner, Deloitte",
    category: "plugin",
    enabled: false,
  },
  {
    name: "currency_exchange"   as const,
