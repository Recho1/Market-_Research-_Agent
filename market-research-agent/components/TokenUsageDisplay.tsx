...
function StatCard({
  ...
  return (
    <div className="bg-[#111118] border border-[#1e1e2e] rounded-lg p-2.5">
      <div className={`text-sm font-mono font-semibold ${colors[color]}`}>
        {value}
      </div>
...
export default function TokenUsageDisplay({ stats }: { stats: SessionStats }) {
  ...
  const costStr = cost < 0.001
    ? `$${(cost * 1000).toFixed(3)}m`
    : `$${cost.toFixed(4)}`;

  const avgMs  = stats.avgResponseMs;
  const avgStr = avgMs > 1000
    ? `${(avgMs / 1000).toFixed(1)}s`
    : `${avgMs}ms`;

  const tokensPerMsg = stats.totalMessages > 0
    ? Math.round(stats.totalTokens / stats.totalMessages)
    : 0;

  const totalRatings = stats.positiveRatings + stats.negativeRatings;
  const satisfaction = totalRatings > 0
    ? Math.round((stats.positiveRatings / totalRatings) * 100)
    : 0;

  const cacheRate = stats.totalMessages > 0
    ? Math.round((stats.cacheHits / stats.totalMessages) * 100)
    : 0;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-1.5">
        <StatCard
          ...
          sub={`${stats.toolCallsCount} tool calls`}
        />
        <StatCard
          label="Est. Cost"
          value={costStr}
        ...
        <StatCard
          ...
          sub={`~${tokensPerMsg}/msg`}
          color="blue"
        />
        <StatCard
          label="Avg Response"
          value={stats.totalMessages > 0 ? avgStr : "--"}
          sub="per message"
          color="green"
        />
      </div>
      ...
      {stats.totalMessages > 0 && (
        <div className="bg-[#111118] border border-[#1e1e2e] rounded-lg p-3 space-y-3">
          ...
          <ProgressBar
            label="Tool Usage"
            value={stats.toolCallsCount}
            max={Math.max(stats.totalMessages * 3, 1)}
            color="#ffb700"
          />
        </div>
      )}
      ...
      {totalRatings > 0 && (
        <div className="bg-[#111118] border border-[#1e1e2e] rounded-lg p-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 text-center">
              <div className="text-sm font-mono text-[#ff6b6b]">
            ...
            <div className="w-px h-8 bg-[#1e1e2e]" />
            <div className="flex-1 text-center">
              <div className="text-sm font-mono text-[#ffb700]">
                {satisfaction}%
              </div>
      ...
      {stats.cacheHits > 0 && (
        <div className="bg-[#111118] border border-[#1e1e2e] rounded-lg p-3">
          ...
          <div className="flex justify-between items-center">
            <span className="text-[11px] text-[#505070]">Cached responses</span>
            <span className="text-[11px] font-mono text-[#7c7cff]">
              {stats.cacheHits}
            </span>
          </div>
          <div className="flex justify-between items-center mt-1">
            <span className="text-[11px] text-[#505070]">Tokens saved</span>
            <span className="text-[11px] font-mono text-[#7c7cff]">
              ~{(stats.cacheHits * tokensPerMsg).toLocaleString()}
            </span>
          </div>
          <div className="flex justify-between items-center mt-1">
            <span className="text-[11px] text-[#505070]">Cost saved</span>
            <span className="text-[11px] font-mono text-[#00ff88]">
              ~${(stats.cacheHits * (cost / Math.max(stats.totalMessages, 1))).toFixed(4)}
            </span>
          </div>
        </div>
      )}
