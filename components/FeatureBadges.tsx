...
const FEATURES = [
  {
    label:"RAG", color:"text-[#7c7cff] bg-[#7c7cff]/8 border-[#7c7cff]/20", dotColor:"bg-[#7c7cff]",
  ...
  {
    label:"Multi-Agent", color:"text-[#ffb700] bg-[#ffb700]/8 border-[#ffb700]/20", dotColor:"bg-[#ffb700]",
  ...
  {
    label:"Memory", color:"text-[#00ff88] bg-[#00ff88]/8 border-[#00ff88]/20", dotColor:"bg-[#00ff88]",
  ...
  {
    label:"PDF Export", color:"text-[#ff6b6b] bg-[#ff6b6b]/8 border-[#ff6b6b]/20", dotColor:"bg-[#ff6b6b]",
      ...
      <div className="flex items-center justify-center gap-2 flex-wrap">
        {FEATURES.map((f,i)=>(
          <button key={i} onClick={()=>setActive(active===i?null:i)}
            className={`flex items-center gap-1.5 text-[9px] font-mono border rounded-full px-2.5 py-1 transition-all duration-200 ${f.color} ${active===i?"ring-1 ring-offset-1 ring-offset-[#0a0a0f] ring-current scale-105":"hover:scale-105"}`}>
          ...
          <div className={`border rounded-xl p-4 bg-[#0d0d14] transition-all ${FEATURES[active].color.split(" ").filter(c=>c.startsWith("border")).join(" ")}`}>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs font-semibold ${FEATURES[active].color.split(" ")[0]}`}>{FEATURES[active].title}</span>
              </div>
              <button onClick={()=>setActive(null)} className="text-[#404060] hover:text-[#e8e8f0] text-xs transition-colors">x</button>
            </div>
            <p className="text-[11px] text-[#a0a0b8] leading-relaxed mb-3">{FEATURES[active].description}</p>
            <div className="flex flex-wrap gap-1">
              {FEATURES[active].stats.split(" · ").map((stat,si)=>(
                <span key={si} className={`text-[9px] font-mono px-2 py-0.5 rounded-full bg-[#111118] border ${FEATURES[active].color.split(" ").filter(c=>c.startsWith("border")).join(" ")} ${FEATURES[active].color.split(" ")[0]}`}>{stat}</span>
              ))}
            </div>
          </div>
        </div>
